...
function get_possible_case_penhoras($penhora = '') {
	$penhoras = array();
	
	$penhoras = array('sem_penhora' => 'Sem Penhora', 
			'bem_movel' => 'Bem Móvel',
			'imovel' => 'Imóvel',
			'veiculo' => 'Veículo', 
			'bacenjud' => 'Bacenjud');
...
function is_penhora_change($type) {
	$penhoras = get_possible_case_penhoras();

	foreach($penhoras as $key => $val)
		if ($key == $type || $val == $type)
			return true;
