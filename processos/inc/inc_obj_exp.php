...
class LcmExpense extends LcmObject {
	...
	function LcmExpense($id_expense = 0) {
		...
		if ($id_expense > 0) {
			$query = "SELECT e.*, a.name_first, a.name_middle, a.name_last, c.p_adverso as case_title
	...
	function save() {
		...
		if ($this->getDataInt('id_expense') > 0) {
			...
			sendmail_despesas("Nova despesa inserida no Sijur.<br> Tipo: "._T('kw__exptypes_' . $this->getDataString('type') . '_title') . "<br> Pasta:" . $this->getDataInt('id_case') ."<br>Link: http://10.10.0.233/despesas/index.php?stt=all&idcase=".$this->getDataInt('id_case')."&tipo=".$this->getDataString('type'),"elizabete.souza@eduardoalbuquerque.adv.br",$this->getDataInt('id_case'),"Nova Despesa");
	...
	function setStatus($new_status) {
		...
		if ($author_session['status'] == 'admin' || $author_session['status'] == 'manager') {
			...
			lcm_query($query);
			//enviando email de alteração do status
			sendmail_despesas("Alteração de Status da despesa no Sijur.<br> Tipo: "._T('kw__exptypes_' . $this->getDataString('type') . '_title') . "<br> Pasta:" . $this->getDataInt('id_case')."<br>Link: http://10.10.0.233/despesas/index.php?stt=all&idcase=". $this->getDataInt('id_case')."&tipo=".$this->getDataString('type'),"elizabete.souza@eduardoalbuquerque.adv.br",$this->getDataInt('id_case'),"Alteração de Status");
...
class LcmExpenseInfoUI extends LcmExpense {
	...
	function printGeneral($full_ui = true) {
		...
		if($this->data['type'] == '_exptypes11')
		{
			echo '<li class="large">'
			. '<span class="label2">' . _Ti('Correspondente') . '</span>' 
			. '<span class="value2">' . nl2br(clean_output($this->getDataString('nome_corresp'))) . '</span>'
			. "</li>\n";
			
			echo '<li class="large">'
			. '<span class="label2">' . _Ti('Data Correspondência') . '</span>' 
			. '<span class="value2">' . nl2br(clean_output($this->getDataString('data_corresp'))) . '</span>'
			. "</li>\n";
		}
	...
	function printComments($full_ui = true) {
		...
		if (! $cpt)
			echo "<p>Nenhum comentário</p>"; // TRAD
	...
	function printEdit() {
		...
		echo '<tr><td>' . f_err_star('cost') . _T('expense_input_cost') . '<span style="float:right"> ' . htmlspecialchars(read_meta('currency')) . '</span></td>' . "\n";
		echo '<td>';
...
class LcmExpenseListUI {
	...
	function start() {
		...
		$headers[$cpt]['title'] = _Th('expense_input_user'); // TRAD
		$headers[$cpt]['order'] = 'no_order';
		$cpt++;

		$headers[$cpt]['title'] = _Th('expense_input_case'); // TRAD
		$headers[$cpt]['order'] = 'no_order';
		$cpt++;
		...
		$headers[$cpt]['title'] = _Th('expense_input_comments'); // TRAD
		$headers[$cpt]['order'] = 'no_order';
		$cpt++;
	...
	function printList() {
		...
		$q = "SELECT e.id_expense, e.id_case, e.id_author, e.status, e.type, 
				...
				count(ec.id_expense) as nb_comments, c.p_adverso as case_title
			FROM lcm_expense as e
		...
		$q .= " GROUP BY e.id_expense, e.id_case, e.id_author, e.status, e.type, e.description, e.date_creation, e.date_update, e.pub_read, e.pub_write, a.name_first, a.name_middle, a.name_last, c.p_adverso ";
		...
		for ($i = 0; (($i<$prefs['page_rows']) && ($row = lcm_fetch_array($result))); $i++) {
			...
			if ($row['id_case'])
				echo '<abbr title="' . $row['case_title'] . '">' . str_pad($row['id_case'], 4, "0", srt_pad_left) . '</a>';
