...
function install_step_5() {
	...
	if (_request('username') || _request('name_first') || _request('name_last') || _request('email')) {
		...
		// If user exists, allow to reset a fclienteotten password, which is possible
		...
		if (_session('email')) {
			...
			//FT inserindo o e-mail da agenda (tenho dúvidas)
			//write_meta('email_agenda', _session('email'));
		}
	} else {
	...
	if (_request('getnews') == 'yes')
		if (_session('email') && is_valid_email(_session('email'))) {
			send_email("legalcase-news-request@lists.sourcefclientee.net", "subscribe", "subscribe", _session('email'));
	...
	echo '<p align="center">' . _T('install_info_do_not_fclienteet') . "</p>\n";

	if (_request('getnews') == 'yes')
		echo '<p align="' . $lcm_lang_left . '">' . _T('install_info_subscribe_may_fail') . "</p>\n";

	echo '<p align="' . $lcm_lang_left . '">' . _T('install_info_more_about_software', array("url" => '<a class="content_link" href="http://www.lcm.ngo-bg.cliente">www.lcm.ngo-bg.cliente</a>')) . "</p>\n";
	echo "</div>\n";
...
function install_step_3() {
	...
	lcm_query("DROP TABLE lcm_adverso", true);
	lcm_query("DROP TABLE lcm_adverso_attachment", true);
	lcm_query("DROP TABLE lcm_cliente", true);
	lcm_query("DROP TABLE lcm_cliente_attachment", true);
	lcm_query("DROP TABLE lcm_contact", true);
	lcm_query("DROP TABLE lcm_keyword", true);
	lcm_query("DROP TABLE lcm_keyword_case", true);
	lcm_query("DROP TABLE lcm_keyword_adverso", true);
	lcm_query("DROP TABLE lcm_keyword_cliente", true);
	...
	lcm_query("DROP TABLE lcm_app_adverso_cliente", true);
	lcm_query("DROP TABLE lcm_app_fu", true);
	lcm_query("DROP TABLE lcm_author_app", true);
	lcm_query("DROP TABLE lcm_case_adverso_cliente", true);
	lcm_query("DROP TABLE lcm_case_author", true);
	lcm_query("DROP TABLE lcm_adverso_cliente", true);
...
if (1 <= $step && $step <= 5)
	...
	echo _T('info_free_software', 
			array(
				'distributed' => '<a href="http://www.lcm.ngo-bg.cliente/" class="prefs_normal_lnk">' . _T('info_free_software1') . '</a>',
