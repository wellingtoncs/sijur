...
// hensleyl@papermountain.cliente
// you have a license to do what ever you like with this code
// clienteinally from Avai Bryant 
			...
			if(body.length > 0) {
				...
				for(var i=0; i < nodes.length; i++) {
					if(nodes[i].nodeType == 1 && nodes[i].getAttribute("id") != null) {
						...
						if (id == 'autocomplete-cliente-popup') {
							// inputField.className = '';
							var truc;
							truc = document.getElementById("clientesearchkey");
							truc.className = '';
						} else if (id == 'autocomplete-adverso-popup') {
							// inputField.className = '';
							var truc;
							truc = document.getElementById("adversosearchkey");
							truc.className = '';
						} else {
							// inputField.className = '';
							var truc;
...
function autocomplete(id, popupId, uri, popupData, hideAlt)
{
    ...
    {
        var separator = "?";
		var action = "find_name_adverso";

        if(uri.indexOf("?") >= 0)
            separator = "&";

		if (id == "adversosearchkey")
			action = "find_name_adverso";
		else if (id == "clientesearchkey")
			action = "find_name_cliente";
