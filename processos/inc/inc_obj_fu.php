...
class LcmFollowup extends LcmObject {
	...
	function save() {
		...
		$fl = " date_start = '" . $this->getDataString('date_start') . "',
				...
				system_name = '" . $this->getDataString('system_name') . "',
		...
		if ($this->getDataInt('id_followup') > 0) {
			...
			$fl .= ", id_author = '{$GLOBALS['author_session']['id_author']}' ";
			...
			//Inserindo o andamento na nova tabela
			$new_fu = lcm_query("select id_case from lcm_followup_last where id_case = " . $this->getDataInt('id_case'));
			$new_wl = lcm_num_rows($new_fu);
			if($new_wl==0)
			{
				lcm_query("INSERT INTO lcm_followup_last SET id_case=" . $this->getDataInt('id_case') . ",type_followup='" . $this->getDataString('type') . "', desc_followup = '" . $this->getDataString('description') . "', date_creat='" . $this->getDataString('date_start') . "'");
			}
			else
			{
				lcm_query("UPDATE lcm_followup_last SET type_followup='" . $this->getDataString('type') . "',desc_followup = '" . $this->getDataString('description') . "', date_creat='" . $this->getDataString('date_start') . "' WHERE id_case=" . $this->getDataInt('id_case') . "");			
			}
...
class LcmFollowupInfoUI extends LcmFollowup {
	...
	function printGeneral($show_subtitle = true, $allow_edit = true) {
		...
		//FT Ocultando a data final do acompanhamento (followup), bem como, o "tempo gasto..."
		echo '<li>'
	...
	function printEdit() {
		...
		//FT Liberando as permissões para os usuários - removido: $dis = isDisabled(!($admin || $edit));
		$dis = ""; //FT isDisabled(($admin || $edit));
		...
		//Liberando alteração da data para todos os usuários
		$name = (($admin || $edit) ? 'start' : 'start');
		...
		if ($this->show_conclusion && _request('submit') == 'set_status') {
		...
		//Atualizar sistemas legem e srs 
		echo "<tr><td>";
		echo _T('fu_input_system_name') . ':';
		echo "</td>\n";
		echo '<td>';
		echo '<input type="radio" name="system_name" value="NOT" checked="checked" />Nenhum';
		echo '<input type="radio" name="system_name" value="LEG" />Legem';
		echo '<input type="radio" name="system_name" value="SRS" />SRS';
		echo "</td>\n";
		echo "</tr>\n";	
		
		// Sum billed field
		//FT alteração do símbolo monetário "R$" posto à esquerda
		if ($this->show_sum_billed == "yes") {
			echo '<tr>';
			echo '<td>' . _T('fu_input_sum_billed');
			...
			echo '<span style="float: right">' . htmlspecialchars($currency) . '</span>';
			echo "</td>\n";
			echo '<td>';
