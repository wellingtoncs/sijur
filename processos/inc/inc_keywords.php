...
$GLOBALS['legal_obj'] = array('case' => 1, 'stage' => 1, 'adverso' => 1, 'cliente' => 1, 'followup' => 1);

//
// get_kwg_all: Returns all keyword groups (kwg) of a given
// type. If type is 'user', then all keyword groups of type
// case, followup, adverso, cliente and author are returned.
...
function get_kwg_all($type, $exclude_empty = false, $show_subgroups = false) {
	...
	if ($type == 'user')
		$in_type = "IN ('case', 'stage', 'followup', 'adverso', 'cliente', 'adverso_cliente')";
	elseif ($type)
...
function get_kwg_applicable_for($type_obj, $id_obj, $id_obj_sec = 0) {
	...
	if ($type_obj == 'adverso' || $type_obj == 'cliente')
		$query .= " OR type = 'adverso_cliente' ";
...
// Return a list of keywords attached to type (case/adverso/cliente/..) for a given ID
		...
		for ($cpt = 0; isset($kw_entries[$cpt]); $cpt++) {
			if ($_REQUEST['kw_del_' . $type_obj . $cpt] || empty($kw_values[$cpt])) {
				if ($type_obj == 'stage') {
					...
					// The id_$type_obj (ex: case, adverso, cliente) is specified
