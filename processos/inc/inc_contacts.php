...
// type_person should be of the enum in the database (author, adverso, cliente, ..)
...
function add_contact($type_person, $id_person, $type_contact, $value, $extra="") {
	if (! $id_person)
	...
	$query = "INSERT INTO lcm_contact (type_person, id_of_person, type_contact, value, date_update, extra)
		VALUES('" . addslashes($type_person) . "', " . intval($id_person) . ", "
			. intval($type_contact) . ", " . "'" . addslashes($value) . "', NOW(),'".$extra."' )";
...
function update_contact($id_contact, $new_value, $extra="") {
	if (! $id_contact)
...
function show_existing_contact($c, $num) {
	...
	// of the user/adverso/cliente, and the value of a contact was changed, then
	...
	echo '<input name="contact_value[]" id="contact_value_' . $num . '" type="text" '
		. 'class="search_form_txt autocomplete_extra" size="35" value="' 
		. (isset($_SESSION['form_data']['contact_value'][$num]) ?  $_SESSION['form_data']['contact_value'][$num] : clean_output($c['value']))
		. '"/>';
		if($c['name']=="+address_main"){
			echo '<input type="text" id="new_extra" name="new_contact_extra[]" class="search_form_txt" style="width:155px">';
			//echo '<button type="button" onclick="pegar_latlon(' . $num_new . ');" class="simple_form_btn">Map</button>';
		}
...
function show_new_contact($num_new, $type_person, $ctype = "__add__", $exceptions = array()) {
	...
	if ($ctype == "__add__") {
		...
		echo '<input name="new_contact_value[]" id="new_contact_value_' . $num_new . '" type="text" '
			. 'class="search_form_txt autocomplete_extra" size="35" value="' . $value . '"/>&nbsp;';
		if($c['name']=="+address_main"){
			echo '<input type="text" id="new_extra" name="new_contact_extra[]" class="search_form_txt" style="width:155px">';
			//echo '<button type="button" onclick="pegar_latlon(' . $num_new . ');" class="simple_form_btn">Map</button>';
		}
...
function update_contacts_request($type_person, $id_of_person) {
	...
	if (isset($_REQUEST['contact_value'])) {
		...
		$new_extra = $_REQUEST['new_contact_extra'];
		//
		...
		for ($cpt = 0; isset($c_ids[$cpt]); $cpt++) {
			...
			if (_request('del_contact_' . $c_ids[$cpt])) {
				...
				if ((! $delete_allowed) && (! $contacts[$cpt])) {
					...
					$err = update_contact($c_ids[$cpt], $contacts[$cpt],$new_extra[$cpt]);
		...
		while (isset($new_contacts[$cpt])) {
			...
			if ($new_contacts[$cpt]) {
				...
				if ($c_type_names[$cpt]) {
					...
					$err = add_contact($type_person, $id_of_person, $c_type_names[$cpt], $new_contacts[$cpt],$new_extra[$cpt]);
