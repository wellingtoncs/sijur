	...
	if (isset($_GET['followup'])) {
		...
		if (empty($_SESSION['errors'])) {
			...
			if (! empty($app)) {
				...
				// Get appointment adverso(s)
				$q = "SELECT aco.*, c.name_first, c.name_middle, c.name_last, o.name
					FROM lcm_app_adverso_cliente as aco
					NATURAL JOIN lcm_adverso as c
					LEFT JOIN lcm_cliente as o ON (o.id_cliente = aco.id_cliente)
					WHERE (id_app = $app AND aco.id_adverso = c.id_adverso)";

				$res_adverso = lcm_query($q);

				if (lcm_num_rows($res_adverso)>0) {
					while ($adverso = lcm_fetch_array($res_adverso))
						$participants[] = get_person_name($adverso)
