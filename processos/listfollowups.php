...
if (strlen($find_case_string) > 0) {
	$q .= " AND ( (c.id_case LIKE '%$find_case_string%')
				OR (c.p_adverso LIKE '%$find_case_string%') )";
}
...
echo '<p><a href="edit_adverso.php" class="create_new_lnk">' . _T('adverso_button_new') . "</a></p>\n";
*/
//-----------------------------------------------
...
// Prepare query
$q = "SELECT c.id_case, c.p_adverso, c.status, c.date_creation,
		fu.*,
...
$q = "SELECT fu.id_case, fu.id_followup, fu.date_start, fu.date_end, fu.type, fu.description,
			a.name_first, a.name_middle, a.name_last, c.p_adverso 
...
for ($i = 0 ; (($i<$prefs['page_rows']) && ($row = lcm_fetch_array($result))); $i++) {
	...
	if ($row['id_case'] != $c) {
		...
		echo '#' . $row['id_case'] . " - '" . $row['p_adverso'] . "'";
