...
function show_report_filters($id_report, $is_runtime = false) {
		...
		while ($filter = lcm_fetch_array($result)) {
			...
			switch ($filter['type']) {
				...
				case 'num_neq':
					if ($filter['field_name'] == 'id_author') {
						...
						//FT incluído manager
						$q = "SELECT * FROM lcm_author WHERE status IN ('admin', 'normal', 'manager', 'external')";
			...
			if (! $is_runtime) {
				...
				echo "<td><a class='content_link' href='upd_rep_field.php?rep=" . $id_report . "&amp;"
					. "remove=filter" . "&amp;" . "id_filter=" . $filter['id_filter'] . "'>" . "<img src='images/jimmac/delete.png' />" . "</a></td>\n";
			}
	...
	if ($rep_info['col_src_type'] == 'keyword' && $rep_info['col_src_name']) {
		...
		if ($kwg['type'] == 'system') {
			...
			if ($kwg['type'] == 'adverso_cliente') {
				array_push($sources, "'lcm_adverso'");
				array_push($sources, "'lcm_cliente'");
			} else {
