...
//
// Copy adversos from the old to the new case
//

// 1. Get the adversos of old case, which are NOT in the new
$q = "SELECT c1.id_case,c1.id_adverso
		 FROM lcm_case_adverso_cliente c1
		 LEFT JOIN lcm_case_adverso_cliente c2
		 ON (c1.id_case=$case) AND (c2.id_case=$destination) AND (c1.id_adverso=c2.id_adverso)
		 WHERE (c1.id_case=$case) AND (c2.id_case IS NULL) AND (c1.id_adverso>0)";
$result = lcm_query($q);

// 2. Associate adversos with the new case
$q = '';
while ($row = lcm_fetch_array($result)) {
	$q .= ($q ? ',' : '') . "($destination," . $row['id_adverso'] . ',DEFAULT)';
}
$q = "INSERT INTO lcm_case_adverso_cliente VALUES $q";
$result = lcm_query($q);

//
// Copy clienteanisations from the old to the new case
//

// 1. Get the clienteanisations of old case, which are NOT in the new
$q = "SELECT o1.id_case,o1.id_cliente
		 FROM lcm_case_adverso_cliente o1
		 LEFT JOIN lcm_case_adverso_cliente o2
		 ON (o1.id_case=$case) AND (o2.id_case=$destination) AND (o1.id_cliente=o2.id_cliente)
		 WHERE (o1.id_case=$case) AND (o2.id_case IS NULL) AND (o1.id_cliente>0)";
$result = lcm_query($q);

// 2. Associate clienteanisations with the new case
$q = '';
while ($row = lcm_fetch_array($result)) {
	$q .= ($q ? ',' : '') . "($destination,DEFAULT," . $row['id_cliente'] . ')';
}
$q = "INSERT INTO lcm_case_adverso_cliente VALUES $q";
$result = lcm_query($q);
