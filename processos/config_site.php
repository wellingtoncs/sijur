...
function show_config_form_general() {
	...
	$email_agenda 	= read_meta('email_agenda');
	...
	echo "<fieldset class='conf_info_box'>\n";
	show_page_subtitle(_T('siteconf_subtitle_site_agenda'), 'siteconfig_general', 'contacts');
	
	echo "<p><b><label for='email_agenda'>" . _T('siteconf_input_email_agenda') . "</label></b></p>\n";
	echo "<p><small class='sm_11'>" . _T('siteconf_info_email_agenda') . "</small></p>\n";
	echo "<p><input type='text' id='email_agenda' name='email_agenda' value='$email_agenda' size='40' class='search_form_txt' /></p>\n";
	echo "<p align='$lcm_lang_right'><button type='submit' name='Validate' id='Validate2' class='simple_form_btn'>" .  _T('button_validate') . "</button></p>\n";
	echo "</fieldset>\n";
}
...
function show_config_form_policy() {
	...
	$meta = array (
		'adverso_name_middle' => '',	'person_name_format' => '',
		'adverso_citizen_number' => '',	'adverso_civil_status' => '',
		'adverso_income' => '', 		'adverso_date_birth' => '',
		'case_assignment_date' => '', 	'case_comarca' => '',
		'case_legal_reason' => '', 		'case_allow_modif' => '',
		'fu_sum_billed' => '', 			'fu_allow_modif' => '',
		'hide_emails' => '',			'case_new_showcliente' => '',  
		'adverso_cpfcnpj' => '', 		'case_p_cliente' => '',
	);
	...
	// ** CLIENTS
	echo "<fieldset class='conf_info_box'>\n";
	show_page_subtitle(_T('siteconf_subtitle_adverso_fields'), 'siteconfig_policy', 'adversos');

	echo '<p><small class="sm_11">' . _T('siteconf_info_adverso_fields') . "</small></p>\n";

	echo '<table width="99%" class="tbl_usr_dtl">' . "\n";
	echo '<tr><td width="300">' . _T('siteconf_input_name_middle') ."</td>\n"
		. "<td>" . get_yes_no_mand('adverso_name_middle', $meta['adverso_name_middle']) .  "</td>\n"
		. "</tr>\n";

	echo "<tr><td>" . _Ti('siteconf_input_citizen_number') ."</td>"
		. "<td>" . get_yes_no_mand('adverso_citizen_number', $meta['adverso_citizen_number']) . "</td>"
		. "</tr>\n";

	echo "<tr><td>" . _Ti('siteconf_input_cpfcnpj') ."</td>"
		. "<td>" . get_yes_no_mand('adverso_cpfcnpj', $meta['adverso_cpfcnpj']) . "</td>"
		. "</tr>\n";

	echo "<tr><td>" . _Ti('person_input_date_birth') ."</td>"
		. "<td>" . get_yes_no_mand('adverso_date_birth', $meta['adverso_date_birth']) . "</td>"
		. "</tr>\n";

	echo "<tr><td>" . _Ti('person_input_civil_status') ."</td>"
		. "<td>" . get_yes_no_mand('adverso_civil_status', $meta['adverso_civil_status']) . "</td>"
		. "</tr>\n";

	echo "<tr><td>" . _Ti('siteconf_input_adverso_income') ."</td>"
		. "<td>" . get_yes_no_mand('adverso_income', $meta['adverso_income']) . "</td>"
		. "</tr>\n";
	...
	echo "<tr><td> " . _Ti('case_input_comarca') ."</td>"
		. "<td>" . get_yes_no_mand('case_comarca', $meta['case_comarca']) . "</td></tr>\n";
	echo "<tr><td> " . _Ti('case_input_legal_reason') ."</td>"
		. "<td>" . get_yes_no_mand('case_legal_reason', $meta['case_legal_reason']) . "</td></tr>\n";
	echo "<tr><td> " . _Ti('case_input_p_cliente') ."</td>"
		. "<td>" . get_yes_no_mand('case_p_cliente', $meta['case_p_cliente']) . "</td></tr>\n";
	echo "<tr><td> " . _T('siteconf_input_case_new_showcliente') ."</td>"
		. "<td>" . get_yes_no('case_new_showcliente', $meta['case_new_showcliente']) . "</td></tr>\n";
...
function apply_conf_changes_general() {
	...
	$email_agenda = _request('email_agenda');
	...
	//FT Criando o e-mail da agenda
	if (! empty($email_agenda)) {
		$old_eagenda = read_meta('email_agenda');
		if ($email_agenda != $old_eagenda) {
			if (is_valid_email($email_agenda)) {
				write_meta('email_agenda', $email_agenda);
				array_push($log, _Ti('siteconf_input_admin_email')
					. now_and_before(clean_input($email_agenda), $old_eagenda));
			} else {
				// FIXME not the best way of showing errors... 
				array_push($log, "Sysadmin e-mail address <tt>"
					. addslashes($email_agenda) . "</tt> is <b>not</b> a "
					. "valid address. Modification not applied."); // TRAD
			}
		}
	}
...
function apply_conf_changes_collab() {
	...
	if ($case_default_read != read_meta('case_default_read')) {
		...
		$entry = "Acesso de leitura para os casos definidos para '<tt>"; // TRAD
	...
	if ($case_default_write != read_meta('case_default_write')) {
		...
		$entry = "Acesso de gravação para os casos definidos para '<tt>"; // TRAD
	...
	if ($case_read_always != read_meta('case_read_always')) {
		...
		$entry = "Acesso para litura podem por alterado apenas por <tt>"; // TRAD
		if ($case_read_always == 'yes') $entry .= "admin ";
	...
	if ($case_write_always != read_meta('case_write_always')) {
		...
		$entry = "Acesso para escrita podem ser alteradas apenas por <tt>"; // TRAD
		if ($case_write_always == 'yes') $entry .= "admin";
		else $entry .= "everybody";
		$entry .= "</tt>";
		array_push($log, $entry);
	}
	...
	if ($site_open_subscription != $old_site_open_subscription) {
			...
			$site_open_subscription == 'no') 
		{
			...
			array_push($log, "Novo autor de auto-registro alterado para " // TRAD
...
function apply_conf_changes_policy() {
	...
	$items = array('adverso_name_middle'=> 'person_input_name_middle',
				'adverso_citizen_number'=> 'person_input_citizen_number',
				'adverso_cpfcnpj' 		=> 'person_input_cpfcnpj',
				'adverso_date_birth'	=> 'person_input_date_birth',
				'adverso_civil_status'  => 'person_input_civil_status',
				'adverso_income'        => 'person_input_income',
				'hide_emails'           => 'siteconf_info_hide_emails',
				'case_assignment_date'  => 'case_input_date_assigned',
				'case_comarca'  		=> 'case_input_comarca',
				'case_legal_reason'		=> 'case_input_legal_reason',
				'case_p_cliente'		=> 'case_input_p_cliente',
				'case_new_showcliente'	=> 'siteconf_input_case_new_showcliente',
