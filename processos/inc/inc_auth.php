...
function auth() {
	...
	if (isset($_COOKIE['lcm_session'])) {
		if (verifier_session($_COOKIE['lcm_session'])) {
			if ($author_session['status'] == 'admin' OR $author_session['status'] == 'normal' OR $author_session['status'] == 'manager') {
	...
	if ($row = lcm_fetch_array($result)) {
		...
		// brutally (with cookie_admin=no to fclienteet the username).
