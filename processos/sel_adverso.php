...
/*
	...
	$Id: sel_adverso.php,v 1.19 2007/03/20 18:33:30 mlutfy Exp $
*/
...
//
// Show only adversos who are not already in the case
// Extract the adversos on the case, then put them in a "not in" list
//
$q = "SELECT *
		FROM lcm_case_adverso_cliente
		WHERE (id_case=$case AND id_adverso>0)";

$result = lcm_query($q);

$q2 = "SELECT id_adverso,name_first,name_middle,name_last
		FROM lcm_adverso
...
if (strlen($find_adverso_string)>1) {
	$q2 .= " AND ((name_first LIKE '%$find_adverso_string%')"
...
lcm_page_start(_T('title_case_add_adverso'));
...
for ($i = 0 ; (($i < $prefs['page_rows']) && ($row = lcm_fetch_array($result))) ; $i++) {
	...
	echo "<input type='checkbox' name='adversos[]' value='" . $row['id_adverso'] . "'>";
	echo "</td>\n";
	...
	echo highlight_matches(clean_output($row['name_first'] . ' ' . $row['name_middle'] . ' '
		. $row['name_last']), $find_adverso_string);
	echo "</a>";
	echo "</td>\n";
...
echo "<tr>\n";
echo '<td colspan="2"><p><a href="edit_adverso.php?attach_case=' . $case . '" class="create_new_lnk">' 
	. _T('adverso_button_new_for_case')
	.  '</a></p></td>' . "\n";
echo "</tr>\n";
...
?>
	...
	<input type="hidden" name="ref_sel_adverso" value="<?php echo $_SERVER['HTTP_REFERER']; ?>">
