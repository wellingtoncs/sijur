...
function get_table_suffix($table) {
	...
	elseif ($table == 'lcm_case')
		return "c";
	elseif ($table == 'lcm_adverso')
		return "cl";
	elseif ($table == 'lcm_case_adverso_cliente')
		return "cco";
...
function join_tables($table1, $table2 = '', $id1 = 0, $id2 = 0, $report = null, $query = '') {
	...
	$table_keys = array(
		...
		"lcm_app" => "id_app",
		"lcm_adverso" => "id_adverso",
		"lcm_cliente" => "id_cliente",
	...
	switch($table1) {
		case 'lcm_author':
			switch($table2) {
				...
				//FT
				case 'lcm_app':
					$where .= " a.id_author = ap.id_author ";
					break;
		...
		case 'lcm_followup':
			switch($table2) {
				...
				case 'lcm_author':
					...
					break;
			//FT implementando a ligação do lcm_followup com lcm_case
				case 'lcm_case':
					$where .= " fu.id_author = a.id_author ";
					break;
				default:
		...
		case 'lcm_app':
			switch($table2) {
				case '':
					break;
				case 'lcm_author':
					$where .= " ap.id_author = a.id_author ";
					break;
				case 'lcm_case':
					$where .= " c.id_case = ap.id_case ";
					break;
		...
		case 'lcm_adverso':
			switch($table2) {
				case '':
					break;
				case 'lcm_case':
					$from   = " , lcm_case_adverso_cliente as cco ";
					$from_glue .= " cl.id_adverso = cco.id_adverso AND c.id_case = cco.id_case ";
					$where .= " cl.id_adverso = cco.id_adverso ";
					break;
				default:
					panic_not_implemented($table1, $table2);
			}
...
function show_filters_info($report) {
	...
	foreach ($my_filters as $f) {
		if (! $f['value']) {
			// Valor pode ser fornecido por $ _REQUEST
		...
		// TODO: se field.type == text e field.valueé baseado em palavra-chave ou
		//lista, use tradução se conhecido.
		if ($f['enum_type']) {
...
function get_filters($id_report, $obj_type = '', $obj_name = '') {
	...
	// XXX não sei como lidar com palavras-chave
	if ($obj_type && $obj_name) {
		// situação especial (aplicar um filtro de estágio em uma lista de casos,
		// ex: número de casos mostram concluiu, por autor, por tipo de crime caso)
...
//
// Para cabeçalhos de relatório (usado mais tarde)
//
...
//
// Obter relatório campos de linha, armazenar em $ relatório-> linhas para mais tarde
//

$report->setupReportLines();

//
// Obter relatório colunas campos loja, em $ relatório-> colunas para mais tarde
//
	...
	if ($row['enum_type']) {
		...
		if ($enum[0] == 'keyword') {
			if ($enum[1] == 'system_kwg') {
				// Há um 'bug' no presente, porque os relatórios soma (fu-tipo-tempo) para cada caso,
				// não mostrar palavras-chave escondida, que incluem "case_stage '. Mas é estranho
				// colocar um tempo em case_stage de qualquer maneira, e mostrando "ocultas" palavras-chave não é muito
				// apropriada nesta situação.
				$kws = get_keywords_in_group_name($enum[2]);

				// Obtenha filtros que podem ser aplicadas
				$tmp_my_filters = get_filters_sql($report, 'table', "'" . $row['table_name'] . "'");
				$sql_filter = ($tmp_my_filters ? " AND " . $tmp_my_filters : "");

				foreach ($kws as $k) {
					// Isto é para os vários tipos de system_kwg.
					// Não muito eficiente, porém funciona por agora.
					if ($enum[2] == 'followups') {
						// Cruzamento lcm_followup com qualquer lcm_author ou lcm_case
						...
						// Para cabeçalhos de relatório
						$k['filter_special'] = 'time_length'; // XXX
						//FT
					} elseif ($enum[2] == 'app') {
						$sql = "SELECT sum(IF(UNIX_TIMESTAMP(ap.end_time) > UNIX_TIMESTAMP(ap.start_time), UNIX_TIMESTAMP(ap.end_time)-UNIX_TIMESTAMP(ap.start_time), 0)) FROM lcm_app as ap WHERE type = '" . $k['name'] . "' AND ap.hidden = 'N' " . $sql_filter;
						...
						if ($my_line_table == 'lcm_author') {
							// Lcm_stage cruzamento com lcm_author (conclusões pelo autor)
							$sql = "SELECT count(*) "
				...
				if ($enum[2] == 'followups') {
					...
					$sql = "SELECT sum(IF(UNIX_TIMESTAMP(ap.end_time) > UNIX_TIMESTAMP(fu.start_time), UNIX_TIMESTAMP(ap.end_time)-UNIX_TIMESTAMP(fu.start_time), 0)) FROM lcm_app as ap WHERE ap.hidden = 'N' " . $sql_filter;
...
if ($rep_info['col_src_type'] == 'keyword' && $rep_info['col_src_name'] && ! count($report->getColumns())) {
	...
	//	$report->addWhere("k.id_group = " . $kwg['id_group']);
		...
		foreach ($kws as $kw) {
			...
			if ($my_line_table == 'lcm_author') {
				// TODO: não podemos usar k.id_keyword vez? e soltar lcm_keyword?
				...
				//FT
			} elseif ($my_line_table == 'lcm_app') {
				$report->addHeader(_Th(remove_number_prefix($kw['title'])), 'time_input_length', '', 'time_length');

				$sql = "SELECT sum(IF(UNIX_TIMESTAMP(ap.end_time) > UNIX_TIMESTAMP(ap.start_time), UNIX_TIMESTAMP(ap.end_time)-UNIX_TIMESTAMP(ap.start_time), 0)) "
					. "FROM lcm_keyword_" . $kwg['type'] . " as ka, lcm_app as ap, lcm_case as c "
					. "WHERE ap.id_case = ka.id_case "
					. " AND c.id_case = ap.id_case "
					. " AND ka.id_keyword = " . $kw['id_keyword']
					. $sql_filter;
	...
	if (isset($_REQUEST['show_nokw']) && $_REQUEST['show_nokw']) {
		...
		// [ML] NOTA: se cruzar com o caso lcm_followup palavras-chave,
		// Eu não estou incomodando com o "não tendo nenhuma palavra-chave" coluna
		// Porque estou farto desta confusão (bem, o SELECT para o
		// Linhas dos relatórios precisa ser corrigido para isso) XXX
	...
	if ($my_line_table == 'lcm_author') {
		...
		$sql = "SELECT sum(IF(UNIX_TIMESTAMP(ap.end_time) > UNIX_TIMESTAMP(ap.start_time), UNIX_TIMESTAMP(ap.end_time)-UNIX_TIMESTAMP(ap.start_time), 0)) "
			. "FROM lcm_app as ap, lcm_case as c "
			. "WHERE ap.id_case = c.id_case " . $sql_filter;
	}
...
if ($rep_info['line_src_type'] == 'table') {
	...
	if ($rep_info['col_src_type'] == 'keyword' && $rep_info['col_src_name']) {
		...
		if ($my_line_table == 'lcm_case') {
			...
			$report->addSQL(" LEFT JOIN lcm_keyword_case as kc "
					. " ON (kc.id_case = ap.id_case AND kc.id_keyword IN (" . join(',', $kw_list_id) . ")) ");
			$report->addSQL(" LEFT JOIN lcm_keyword as k ON (k.id_keyword = kc.id_keyword) ");
		}
		...
		default:
			switch($my_col_table) {
				...
				case 'lcm_case':
					...
					break;
				case 'lcm_adverso':
					$report->addSQL(" LEFT JOIN lcm_keyword_adverso as kc ON (kc.id_keyword = k.id_keyword) ");

					if ($my_line_table != 'lcm_adverso')
						$report->addSQL(" LEFT JOIN lcm_adverso as cl ON (cl.id_adverso = kc.id_adverso) ");
					break;
				case 'lcm_stage':
...
if ($rep_info['filecustom']) {
	...
	if (! $result) {
		if (isset($_REQUEST['debug'])) {
			echo "O relatório não pôde ser gerado. Por favor, se você acha que isso é um erro, envie as seguintes informações para os desenvolvedores de software."; // TRAD
			...
			echo "O relatório não pôde ser gerado. Tente executar novamente usando o "
				. '<a href="' . $tmp_link->getUrl() . '">modo de depuração</a>.'; // TRAD
		}
...
foreach ($my_headers as $h)
	$report->printHeaderValue(_T($h['description']));
		...
		if ((! is_numeric($key)) && ($key != 'LCM_HIDE_ID')) {
			...
			if ($val == "1" && preg_match("/^LCM_SQL:special:(.*)/", $key, $regs)) {
				...
				if (isset($_REQUEST['zoom' . $cpt_lines . "-" . $cpt_col])) {
					...
					$q_col = preg_replace("/count\(\*\)/", "c.p_adverso, c.id_case", $q_col);
				}
...
if ($report->getOption('headers_sent') == 'yes') {
	...
	echo "<p>Número de linhas: " . $report->getRowCount() . "</p>\n"; // TRAD

	// Notas de rodapé do relatório (ex: assinado pelo gerente, etc - permitem HTML)
	echo $rep_info['notes'];
	...
	//
	// Faça um link para exportar o relatório
	//
	echo '<p>';
