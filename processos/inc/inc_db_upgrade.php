...
function upgrade_database($old_db_version) {
	...
	// (stores a cookie for when user fclienteets pass and needs reset)
	...
	if ($lcm_db_version_current < 7) {
		...
		// For author/adverso/cliente contact book
		lcm_query("CREATE TABLE lcm_contact (
			id_contact bigint(21) NOT NULL auto_increment,
			type_person ENUM('author', 'adverso', 'cliente') DEFAULT 'author' NOT NULL,
	...
	if ($lcm_db_version_current < 8) {
		...
		lcm_query("ALTER TABLE lcm_author
			CHANGE status status ENUM('admin', 'normal', 'manager', 'external', 'trash', 'waiting', 'suspended')
	...
	if ($lcm_db_version_current < 10) {
		...
		$query = "CREATE TABLE lcm_keyword_group (
			...
			title text NOT NULL,
			description text NOT NULL,
			type ENUM('system', 'case', 'followup', 'adverso', 'cliente', 'author'),
	...
	if ($lcm_db_version_current < 11) {
		write_metas(); // fclienteotten at last upgrade
	...
	if ($lcm_db_version_current < 12) {
		lcm_query("ALTER TABLE lcm_adverso ADD gender ENUM('female', 'male', 'unknown') DEFAULT 'unknown' NOT NULL");
	...
	if ($lcm_db_version_current < 13) {
		...
		lcm_query("REPLACE INTO lcm_fields VALUES (1, 'lcm_case', 'p_adverso', 'Case: Title'),
											...
											(5, 'lcm_case', 'p_cliente', 'Case: P Cliente'),
											(6, 'lcm_case', 'comarca', 'Case: Alleged crime'),
...
/*		lcm_query("CREATE TABLE lcm_filter_conds (
	...
	if ($lcm_db_version_current < 20) {
		...
		lcm_query("INSERT INTO lcm_fields (table_name, field_name, description, enum_type, filter) VALUES
				...
				('lcm_case',     'p_adverso',            'p_adverso',            '', 'text'),
				...
				('lcm_case',     'p_cliente',     	 'p_cliente',     	 '', 'none'),
				('lcm_case',     'comarca',   		 'alleged_crime',    '', 'none'),
				...
				('lcm_adverso',   'id_adverso',        'id_adverso',        '', 'number'),
				('lcm_adverso',   'name_first',       'name_first',       '', 'text'),
				('lcm_adverso',   'name_middle',      'name_middle',      '', 'text'),
				('lcm_adverso',   'name_last',        'name_last',        '', 'text'),
				('lcm_adverso',   'date_creation',    'date_creation',    '', 'date'),
				('lcm_adverso',   'citizen_number',   'citizen_number',   '', 'text'),
				('lcm_adverso',   'cpfcnpj',  		 'cpfcnpj',   		 '', 'text'),
				('lcm_adverso',   'civil_status',     'civil_status',     'keyword:system_kwg:civilstatus', 'number'),
				('lcm_adverso',   'income',           'income',           'keyword:system_kwg:income', 'number'),
				('lcm_adverso',   'gender',           'gender',           'list:female,male,unknown', 'text'),
	...
	if ($lcm_db_version_current < 21) {
		...
		lcm_query("CREATE TABLE lcm_app_adverso_cliente (
			id_app bigint(21) NOT NULL default '0',
			id_adverso bigint(21) NOT NULL default '0',
			id_cliente bigint(21) NOT NULL default '0',
			KEY id_app (id_app,id_adverso,id_cliente))");
	...
	if ($lcm_db_version_current < 23) {
		...
		$tables = array('lcm_app_adverso_cliente' => 'id_app,id_adverso,id_cliente',
				'lcm_app_fu' => 'id_app,id_followup',
				'lcm_author_app' => 'id_author,id_app',
				'lcm_case_adverso_cliente' => 'id_case,id_adverso,id_cliente',
				'lcm_case_author' => 'id_case,id_author',
				'lcm_adverso_cliente' => 'id_adverso,id_cliente',
		...
		lcm_query("ALTER TABLE lcm_app_adverso_cliente DROP INDEX id_app");
		lcm_query("CREATE UNIQUE INDEX uniq ON lcm_app_adverso_cliente (id_app,id_adverso,id_cliente)");
		...
		lcm_query("CREATE UNIQUE INDEX uniq ON lcm_case_adverso_cliente (id_case,id_adverso,id_cliente)");

		lcm_query("CREATE UNIQUE INDEX uniq ON lcm_case_author (id_case,id_author)");

		lcm_query("CREATE UNIQUE INDEX uniq ON lcm_adverso_cliente (id_adverso,id_cliente)");
	...
	if ($lcm_db_version_current < 24) {
		...
		lcm_query("ALTER TABLE lcm_adverso CHANGE civil_status civil_status varchar(255) NOT NULL DEFAULT 'unknown'");
		lcm_query("ALTER TABLE lcm_adverso CHANGE income income varchar(255) NOT NULL DEFAULT 'unknown'");
		lcm_query("UPDATE lcm_adverso SET civil_status = 'unknown', income = 'unknown'");
	...
	if ($lcm_db_version_current < 27) {
		// Add adverso attachments table
		lcm_query("CREATE TABLE lcm_adverso_attachment (
		  id_attachment bigint(21) NOT NULL auto_increment,
		  id_adverso bigint(21) NOT NULL default '0',
		  filename varchar(255) NOT NULL default '',
		  type varchar(255) default NULL,
	...
	if ($lcm_db_version_current < 28) {
		// Add adverso attachments table
		lcm_query("CREATE TABLE lcm_cliente_attachment (
		  id_attachment bigint(21) NOT NULL auto_increment,
		  id_cliente bigint(21) NOT NULL default '0',
		  filename varchar(255) NOT NULL default '',
		  type varchar(255) default NULL,
	...
	if ($lcm_db_version_current < 29) {
		...
		lcm_query("CREATE TABLE lcm_keyword_adverso (
			id_entry bigint(21) NOT NULL auto_increment,
		...
		lcm_query("CREATE TABLE lcm_keyword_cliente (
			id_entry bigint(21) NOT NULL auto_increment,
			id_keyword bigint(21) NOT NULL default '0',
			id_cliente bigint(21) NOT NULL default '0',
			PRIMARY KEY (id_entry),
		...
		lcm_query("ALTER TABLE lcm_case ADD notes text NOT NULL AFTER comarca");
		lcm_query("ALTER TABLE lcm_adverso ADD notes text NOT NULL");

		lcm_query("ALTER TABLE lcm_cliente 
						ADD notes text NOT NULL,
						ADD court_reg text NOT NULL,
						ADD tax_number text NOT NULL,
						ADD stat_number text NOT NULL");

		// Remove lcm_adverso.address = lcm_cliente.address and move to lcm_contacts
		...
		lcm_query("INSERT INTO lcm_contact (type_person, id_of_person, value, type_contact)
				SELECT 'adverso', id_adverso, address, " . $id_address . " 
					FROM lcm_adverso
					WHERE (address IS NOT NULL AND address != '')");

		lcm_query("INSERT INTO lcm_contact (type_person, id_of_person, value, type_contact)
				SELECT 'cliente', id_cliente, address, " . $id_address . " 
					FROM lcm_cliente
					WHERE (address IS NOT NULL AND address != '')");
	...
	if ($lcm_db_version_current < 30) {
		lcm_query("ALTER TABLE lcm_keyword_group
			CHANGE type type ENUM('system','case','followup','adverso','cliente','adverso_cliente','author')");
		...
		lcm_query("ALTER TABLE lcm_keyword_adverso ADD KEY id_keyword (id_keyword)");
		lcm_query("ALTER TABLE lcm_keyword_cliente ADD KEY id_keyword (id_keyword)");

		upgrade_db_version (30); 
	}
	
	if ($lcm_db_version_current < 31) {
		lcm_query("ALTER TABLE lcm_adverso DROP address");
		lcm_query("ALTER TABLE lcm_cliente DROP address");
		...
		lcm_query("ALTER TABLE lcm_case_attachment	ADD id_author BIGINT(21) NOT NULL AFTER id_case,
								...
								ADD INDEX (id_author)");
		lcm_query("ALTER TABLE lcm_adverso_attachment	ADD id_author BIGINT(21) NOT NULL AFTER id_adverso,
								CHANGE content content LONGBLOB DEFAULT NULL,
								ADD date_removed DATETIME NOT NULL,
								ADD INDEX (id_author)");
		lcm_query("ALTER TABLE lcm_cliente_attachment	ADD id_author BIGINT(21) NOT NULL AFTER id_cliente,
								CHANGE content content LONGBLOB DEFAULT NULL,
								ADD date_removed DATETIME NOT NULL,
								ADD INDEX (id_author)");
	...
	if ($lcm_db_version_current < 34) {
		// Add 'stage' type
		lcm_query("ALTER TABLE lcm_keyword_group
					CHANGE type type ENUM('system','case','stage','followup','adverso','cliente','adverso_cliente','author')");
	...
	if ($lcm_db_version_current < 41) {
		...
		// do not activate their "gender" field get adversos that are all male.
		lcm_query("ALTER TABLE lcm_adverso 
					CHANGE gender gender enum('female', 'male', 'unknown') 
					DEFAULT 'unknown' NOT NULL");

		// Therefore, the following "UPDATE" will not fix much, users will have
		// to manually fix their adverso data, but just in case..
		lcm_query("UPDATE lcm_adverso
					SET gender = 'unknown'
					WHERE gender = ''");
	...
	if ($lcm_db_version_current < 43) {
		lcm_query("ALTER TABLE lcm_keyword_adverso
					ADD value text NOT NULL");

		lcm_query("ALTER TABLE lcm_keyword_cliente
					ADD value text NOT NULL");
	...
	if ($lcm_db_version_current < 44) {
		...
		$upd_metas = array('adverso_name_middle', 'adverso_citizen_number', 'adverso_cpfcnpj', 
						'adverso_civil_status', 'adverso_income', 
						'case_comarca', 'case_legal_reason', 'case_p_cliente');
	...
	if ($lcm_db_version_current < 50) {
		...
		lcm_query("ALTER TABLE lcm_adverso_attachment
			CHANGE date_removed date_removed datetime default NULL");

		lcm_query("ALTER TABLE lcm_cliente_attachment
			CHANGE date_removed date_removed datetime default NULL");
	...
	if ($lcm_db_version_current < 51) {
		...
		//
		lcm_query("ALTER TABLE lcm_keyword_group
			CHANGE type type ENUM('system', 'contact', 'case', 'stage', 'followup', 'adverso', 'cliente', 'adverso_cliente', 'author') NOT NULL");
