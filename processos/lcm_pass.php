...
function send_cookie_by_email($my_email) {
	...
	install_html_start(_T('pass_title_fclienteotten_password'), 'login');
	...
	if ($row['status'] == 'trash' OR $row['password'] == '') {
		...
		if (send_email($my_email, "[$site_name] "._T('pass_title_fclienteotten_password'), $message)) {
...
function print_pass_fclienteotten_form() {
	install_html_start(_T('pass_title_fclienteotten_password'), 'login');

	echo "<p>" . _T('pass_info_enter_email') . "</p>\n";

	echo "<form action='lcm_pass.php' name='form_fclienteotten' id='form_fclienteotten' method='post'>\n";
	echo "<input type='hidden' name='pass_fclienteotten' value='yes' />\n";
	...
	echo "<script type=\"text/javascript\"><!--
			document.form_fclienteotten.user_email.focus();
			//--></script>\n";
}
